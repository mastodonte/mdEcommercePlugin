      ...
      <?php foreach ($categorias as $record_categoria): ?>
        ...
        <li>
          <a class="<?php echo ($record_categoria['id'] ==  $categoria->getId() ? 'active' : ''); ?>" 
             href="<?php echo ($hasChildren && false ? 'javascript:void(0)' : url_for('@productos-categoria?id=' . $record_categoria['id'] . '&name=' . mdBasicFunction::slugify($record_categoria['name'])) . '?s=' . base64_encode($parent->getId() . '-' . $parent->getName())); ?>">
