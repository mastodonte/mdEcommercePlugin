    ...
    <table class="table table-hover" id="main_list" cellspacing="0" style="display: none">
      <thead>
        <tr>
          ...
          <?php include_partial('categoriasBackend/list_th_tabular', array('sort' => $sort)) ?>
        ...
        <tr>
          <th colspan="8">
            <?php if ($pager->haveToPaginate()): ?>
              <?php include_partial('categoriasBackend/pagination', array('pager' => $pager)) ?>
            <?php endif; ?>
      ...
      <tbody>
        <?php foreach ($pager->getResults() as $i => $ec_category): $odd = fmod(++$i, 2) ? 'odd' : 'even' ?>
          <tr id="node-<?php echo $ec_category['id'] ?>" class="sf_admin_row <?php echo $odd ?><?php
          // insert hierarchical info
          $node = $ec_category->getNode();
          if ($node->isValidNode() && $node->hasParent())
          {
            echo " child-of-node-".$node->getParent()->getId();
          }
          ?>">
            <?php include_partial('categoriasBackend/list_td_batch_actions', array('ec_category' => $ec_category, 'helper' => $helper)) ?>
            <?php include_partial('categoriasBackend/list_td_tabular', array('ec_category' => $ec_category)) ?>
            <?php include_partial('categoriasBackend/list_td_actions', array('ec_category' => $ec_category, 'helper' => $helper)) ?>
          </tr>
        <?php endforeach; ?>
      </tbody>
    </table>
